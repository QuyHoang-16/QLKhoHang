...
namespace QuanLyKho.Services
{
    public class NetcoreService : INetcoreService
    {
        ...
        public async Task<string> UploadFile(List<IFormFile> files, Microsoft.AspNetCore.Hosting.IHostingEnvironment env)
        {
            var result = "";
        ...
        public VMStock GetStockByProductAndWarehouse(string productId, string warehouseId)
        {
            ...
            try
            {
                Product product = _context.Products.Where(x => x.productId.Equals(productId)).FirstOrDefault();
                Warehouse warehouse = _context.Warehouses.Where(x => x.warehouseId.Equals(warehouseId)).FirstOrDefault();
                if (product != null && warehouse != null)
                {
                    VMStock stock = new VMStock();
                    stock.Product = product.productCode;
                    stock.Warehouse = warehouse.warehouseName;
                    stock.QtyReceiving = _context.ReceivingLines.Where(x => x.productId.Equals(product.productId) && x.warehouseId.Equals(warehouse.warehouseId)).Sum(x => x.qtyReceive);
                    stock.QtyShipment = _context.ShipmentLines.Where(x => x.productId.Equals(product.productId) && x.warehouseId.Equals(warehouse.warehouseId)).Sum(x => x.qtyShipment);
                    stock.QtyTransferIn = _context.TransferInLines.Where(x => x.productId.Equals(product.productId) && x.transferIn.warehouseIdTo.Equals(warehouse.warehouseId)).Sum(x => x.qty);
                    stock.QtyTransferOut = _context.TransferOutLines.Where(x => x.productId.Equals(product.productId) && x.transferOut.warehouseIdFrom.Equals(warehouse.warehouseId)).Sum(x => x.qty);
        ...
        public async Task InitDemo()
        {
            try
            {
                ...
                _context.Warehouses.AddRange(whs);

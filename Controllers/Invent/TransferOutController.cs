...
namespace QuanLyKho.Controllers.Invent
{
    ...
    public class TransferOutController : Controller
    {
        ...
        {
            TransferOut obj = await _context.TransferOuts
        ...
        {
            var applicationDbContext = _context.TransferOuts.OrderByDescending(x => x.createdAt).Include(t => t.transferOrder);
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            var transferOut = await _context.TransferOuts
        ...
        public async Task<IActionResult> Create([Bind("transferOutId,transferOrderId,transferOutNumber,transferOutDate,description,branchIdFrom,warehouseIdFrom,branchIdTo,warehouseIdTo,HasChild,createdAt")] TransferOut transferOut)
        {
            if (ModelState.IsValid)
            {
                ...
                TransferOut check = await _context.TransferOuts
                    .Include(x => x.transferOrder)
                ...
                stocklines = _context.TransferOrderLines
                    .Include(x => x.transferOrder)
                    .Include(x => x.product)
                ...
                lines = _context.TransferOrderLines.Include(x => x.product).Where(x => x.transferOrderId.Equals(transferOut.transferOrderId)).ToList();
                foreach (var item in lines)
                {
                    ...
                    _context.TransferOutLines.Add(line);
                    await _context.SaveChangesAsync();
                }
        ...
        public async Task<IActionResult> DeleteConfirmed(string id)
        {
            ...
            try
            {
                _context.TransferOutLines.RemoveRange(transferOut.transferOutLine);
                _context.TransferOuts.Remove(transferOut);
        ...
        {
            return _context.TransferOuts.Any(e => e.transferOutId == id);
        }

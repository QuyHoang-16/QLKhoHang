...
namespace QuanLyKho.Controllers.Invent
{
    ...
    public class PurchaseOrderLineController : Controller
    {
        ...
        {
            var applicationDbContext = _context.PurchaseOrderLines.Include(p => p.product).Include(p => p.purchaseOrder);
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            var purchaseOrderLine = await _context.PurchaseOrderLines
                    .Include(p => p.product)
        ...
        {
            var check = _context.PurchaseOrderLines.SingleOrDefault(m => m.purchaseOrderLineId == id);
            var selected = _context.PurchaseOrders.SingleOrDefault(m => m.purchaseOrderId == masterid);
        ...
        public async Task<IActionResult> Create([Bind("purchaseOrderLineId,purchaseOrderId,productId,qty,price,discountAmount,totalAmount,createdAt")] PurchaseOrderLine purchaseOrderLine)
        {
            ...
            ViewData["productId"] = new SelectList(_context.Products, "productId", "productId", purchaseOrderLine.productId);
            ViewData["purchaseOrderId"] = new SelectList(_context.PurchaseOrders, "purchaseOrderId", "purchaseOrderId", purchaseOrderLine.purchaseOrderId);
            return View(purchaseOrderLine);
        }
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            var purchaseOrderLine = await _context.PurchaseOrderLines.SingleOrDefaultAsync(m => m.purchaseOrderLineId == id);
            if (purchaseOrderLine == null)
            {
                return NotFound();
            }
        ...
        {
            return _context.PurchaseOrderLines.Any(e => e.purchaseOrderLineId == id);
        }

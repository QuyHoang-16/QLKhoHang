...
namespace QuanLyKho.Controllers.Invent
{
    ...
    public class ShipmentLineController : Controller
    {
        ...
        {
            var applicationDbContext = _context.ShipmentLines.Include(s => s.branch).Include(s => s.product).Include(s => s.shipment).Include(s => s.warehouse);
        ...
        {
            var check = _context.ShipmentLines.SingleOrDefault(m => m.shipmentLineId == id);
            var selected = _context.Shipments.SingleOrDefault(m => m.shipmentId == masterid);
        ...
        public async Task<IActionResult> Create([Bind("shipmentLineId,shipmentId,branchId,warehouseId,productId,qty,qtyShipment,qtyInventory,createdAt")] ShipmentLine shipmentLine)
        {
            ...
            ViewData["branchId"] = new SelectList(_context.Branches, "branchId", "branchId", shipmentLine.branchId);
            ViewData["productId"] = new SelectList(_context.Products, "productId", "productId", shipmentLine.productId);
            ViewData["shipmentId"] = new SelectList(_context.Shipments, "shipmentId", "shipmentId", shipmentLine.shipmentId);
            ViewData["warehouseId"] = new SelectList(_context.Warehouses, "warehouseId", "warehouseId", shipmentLine.warehouseId);
            return View(shipmentLine);
        }
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            var shipmentLine = await _context.ShipmentLines.SingleOrDefaultAsync(m => m.shipmentLineId == id);
            if (shipmentLine == null)
            {
                return NotFound();
            }
        ...
        {
            var shipmentLine = await _context.ShipmentLines.SingleOrDefaultAsync(m => m.shipmentLineId == id);
            _context.ShipmentLines.Remove(shipmentLine);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            return _context.ShipmentLines.Any(e => e.shipmentLineId == id);
        }

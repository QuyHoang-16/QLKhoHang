...
namespace NetCore_Update.Controllers.Invent
{
    ...
    public class ReceivingLineController : Controller
    {
        ...
        {
            var applicationDbContext = _context.ReceivingLines.Include(r => r.branch).Include(r => r.product).Include(r => r.receiving).Include(r => r.warehouse);
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            var receivingLine = await _context.ReceivingLines
                    .Include(r => r.branch)
                    .Include(r => r.product)
        ...
        {
            var check = _context.ReceivingLines.SingleOrDefault(m => m.receivingLineId == id);
            var selected = _context.Receivings.SingleOrDefault(m => m.receivingId == masterid);
            ...
            ViewData["receivingId"] = new SelectList(_context.Receivings, "receivingId", "receivingNumber");
        ...
        public async Task<IActionResult> Create([Bind("receivingLineId,receivingId,branchId,warehouseId,productId,qty,qtyReceive,qtyInventory,createdAt")] ReceivingLine receivingLine)
        {
            ...
            ViewData["branchId"] = new SelectList(_context.Branches, "branchId", "branchName", receivingLine.branchId);
            ViewData["productId"] = new SelectList(_context.Products, "productId", "productCode", receivingLine.productId);
            ViewData["receivingId"] = new SelectList(_context.Receivings, "receivingId", "receivingNumber", receivingLine.receivingId);
            ViewData["warehouseId"] = new SelectList(_context.Warehouses, "warehouseId", "warehouseName", receivingLine.warehouseId);
            return View(receivingLine);
        }
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            var receivingLine = await _context.ReceivingLines.SingleOrDefaultAsync(m => m.receivingLineId == id);
            if (receivingLine == null)
            {
                return NotFound();
            }
        ...
        {
            var receivingLine = await _context.ReceivingLines.SingleOrDefaultAsync(m => m.receivingLineId == id);
            _context.ReceivingLines.Remove(receivingLine);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            return _context.ReceivingLines.Any(e => e.receivingLineId == id);
        }
